...
Original file is located at
    https://colab.research.google.com/drive/1HPejCRndz2tl46Kd8yTF2_vTv75Lpn0S

# Neural Machine Translation for En-Hi
...
"""Create a working directory and change the location (where bpe_codes and trained model are present)
...
cd /content/gdrive/MyDrive/NASSCOM_HI/
...
Downloading "PMIndia" Dataset, PMIndia dataset consists of parallel sentences for 13 language pairs.
"""

!wget -c "http://data.statmt.org/pmindia/v1/parallel/pmindia.v1.hi-en.tsv"
...
import pandas as pd
tsv_read = pd.read_csv("pmindia.v1.hi-en.tsv", sep='\t',names=["EN", "HI"])
src=tsv_read['EN'].tolist()
tgt=tsv_read['HI'].tolist()

with open('pmi-en.txt', 'w') as f:
    for item in src:
        f.write("%s\n" % item)

with open('pmi-hi.txt', 'w') as f:
    for item in tgt:
        f.write("%s\n" % item)
...
Byte pair encoding is a subword segmentation algorithm which is used to break the words into sub-words. The following example illustrates the BPE merge operations:
1. 
```
...
**Create instances of normalizer and tokenizer for english and hindi**
"""
...
"""**Preprocess the data (Normalization + Tokenization)**"""
...
for i in tgt:
    preprocessed_tgt+=[preprocess_hi(i)]
...
with open('train-hi.txt', 'w') as f:
    for item in preprocessed_tgt_train:
        f.write("%s\n" % item)

"""**Learn BPE (Byte Pair Encoding)**

Learn BPE merges (Shown here on a small number of sentences with a small number of BPE merge operations).

Number of BPE merge operations is an important parameter here.

"""

!subword-nmt learn-bpe -s 250 < pmi-hi.txt > pmi-bpe-codes-hi.txt #Change number of merge operations
!subword-nmt learn-bpe -s 250 < pmi-en.txt > pmi-bpe-codes-en.txt #Change number of merge

"""**Apply BPE**

We have already stored BPE_codes which are trained on the complete data (and which are being utilized here).
"""

!subword-nmt apply-bpe -c bpe-codes.hi < train-hi.txt > bpe-train-hi.txt
!subword-nmt apply-bpe -c hi-bpe-codes.en < train-en.txt > bpe-train-en.txt

"""**Sample Training Data (Before and after applying preprocessing)** 

Let's look at the dataset before and after preprocessing.
"""

!head -n 5 train-hi.txt

!head -n 5 bpe-train-hi.txt
...
This module creates vocabulary based on provided source and target text files and stores data in the form of tensors.
"""

!onmt_preprocess \
  -train_src bpe-train-en.txt -train_tgt bpe-train-hi.txt \
  -save_data my_train_data

"""**Training Model**

Here, we show training on a small sample of data for 20 steps over an already trained model (which is trained using various different sources of parallel data). All details of data sources and model is given in the end.
"""

!onmt_train \
  -data my_train_data \
  -save_model my_model \
  -train_from hi-model.pt \
  ...
  -train_steps 140020 -max_generator_batches 2 \
...
# %%capture 
# !subword-nmt apply-bpe -c hi-bpe-codes.en < processed-test-pmi.txt > bpe-processed-test-pmi.txt
# !onmt_translate -model hi-model.pt -src bpe-processed-test-pmi.txt -output bpe-trans-pmi.txt -replace_unk -gpu 0
...
with open('trans-pmi.txt','r') as a,open('detok-trans-pmi.txt','w') as b:
    for item in a:
        b.write("%s" % indic_detokenize.trivial_detokenize(item,lang='hi'))
...
# Commented out IPython magic to ensure Python compatibility.
# %%capture
# bleu = score("test-pmi.hi","detok-trans-pmi.txt")

print(bleu)

"""**Translation/Inference demo**"""
...
def translate_hi(s):
    ...
    with open('sentence.txt','w') as f:
        f.write(s)
    #Apply BPE
    !subword-nmt apply-bpe -c hi-bpe-codes.en < sentence.txt > bpe-sentence.txt
    #Translate using OpenNMT
    !onmt_translate -model hi-model.pt -src bpe-sentence.txt -output bpe-trans.txt -replace_unk -gpu 0
    #De-BPE
    ...
    with open('trans.txt') as f:
        for i in f:
            out+=i
    return indic_detokenize.trivial_detokenize(out,lang='hi')
